...
const DEFAULT_GLOW_COLOR = "198, 241, 53";
...
export default function MagicBento({
  ...
  const baseClassName = (accent?: boolean) =>
    [
      "magic-bento-card",
      textAutoHide ? "magic-bento-card--text-autohide" : "",
      enableBorderGlow ? "magic-bento-card--border-glow" : "",
      accent ? "magic-bento-card--accent" : "",
    ]
      .filter(Boolean)
      .join(" ");

  const CardContent = ({ card }: { card: BentoCard }) => (
    <>
      <div className="magic-bento-card__header">
        <div className="magic-bento-card__label">{card.label}</div>
      </div>
      <div className="magic-bento-card__content">
        {card.stat && <div className="magic-bento-card__stat">{card.stat}</div>}
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cards.map((card, index) =>
          enableStars ? (
            <ParticleCard
              key={index}
              className={baseClassName(card.accent)}
              style={{ backgroundColor: card.color ?? "#111111", ["--glow-color" as string]: glowColor }}
              disableAnimations={shouldDisableAnimations}
